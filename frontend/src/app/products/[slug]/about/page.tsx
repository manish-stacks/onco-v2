...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const url = absoluteUrl(`/products/${slug}/about`);
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:px-8">
      <p className="mb-6 flex items-center gap-1.5 text-xs font-medium text-[var(--ink-soft)]">
        ...
        <Link href={`/products/${slug}`} className="hover:text-[var(--blue-600)]">{category.category_name}</Link>
        <ChevronRight size={12} />
      ...
      <div className="mt-10 flex flex-wrap gap-3 border-t border-[var(--line)] pt-6">
        <Button href={`/products/${slug}`} icon={<ShoppingBag size={16} />}>
